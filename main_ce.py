...
def parse_option():
    ...
    # optimization
    parser.add_argument(
        "--learning_rate", type=float, default=0.2, help="learning rate"
    )
    parser.add_argument(
        "--lr_decay_epochs",
        type=str,
...
def set_loader(opt):
    ...
    if opt.dataset == "cifar10":
        ...
        val_dataset = datasets.CIFAR10(
            root=opt.data_folder, train=False, transform=val_transform
        )
    elif opt.dataset == "cifar100":
        ...
        train_dataset = datasets.ImageFolder(
            root=opt.data_folder, transform=train_transform
        )
        val_dataset = datasets.ImageFolder(
            root=opt.val_folder, transform=train_transform
        )
    else:
    ...
    val_loader = torch.utils.data.DataLoader(
        val_dataset, batch_size=256, shuffle=True, num_workers=opt.num_workers, pin_memory=True
    )
